...
Correlation <- function(){
  ...
  time <- system.time(model <- caret::train(pathology~., data=train, method="svmLinear", preProcess="scale", trControl=control, metric = "Kappa"))
...
ChiSquare <-function(){
  ...
  set.seed(123)
  model <- makeLearner("classif.ksvm", predict.type = "prob")
  ...
  res = tuneParams(lrn, task = train_task, resampling = rdesc, par.set = ps, measures = list(mlr::acc,mlr::kappa,kappa_sd),
...
SequentialForward <-function(){
  ...
  kappa_sd <- setAggregation(kappa,test.sd)
  rdesc <- makeResampleDesc("CV", iters = 10)
  lrn <-  makeFeatSelWrapper(model, resampling = rdesc,measures = list(mlr::acc,mlr::kappa,kappa_sd),
