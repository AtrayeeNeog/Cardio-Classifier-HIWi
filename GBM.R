...
InfoGain <-function(){
  ...
  rdesc = makeResampleDesc("CV", iters = 10)
  res = tuneParams(lrn, task = train_task, resampling = rdesc, par.set = ps,
                   control = makeTuneControlGrid())
...
Correlation <- function(){
  ...
  time <- system.time(model <- caret::train(pathology~., data=train, method="gbm", preProcess="scale", trControl=control, metric = "Kappa"))
