...
Correlation <- function(){
  ...
  time <- system.time(model <- caret::train(pathology~., data=train, method="gbm", preProcess="scale", trControl=control))
...
Genetic <-function(){
  ...
  rdesc = makeResampleDesc("CV", iters=10)
  kappa_sd <- setAggregation(kappa,test.sd)
  lrn = makeFeatSelWrapper(model, resampling = rdesc,measures = list(acc,kappa,kappa_sd),
