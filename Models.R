...
Decision_Tree <- function(){
  # Deciding which Feature Selection Method to take data from:
  {
    ...
    if(option==6){
      data <- readr::read_csv(here::here("Data","Features-With-GeneticAlgorithm.csv"))
      # Data Slicing:
      set.seed(3033)
      intrain <- createDataPartition(y = data$pathology, p= 0.7, list = FALSE)
      training <- data[intrain,]
...
Lasso <- function(){
  ...
  y_train = ifelse(training$pathology == "healthy", 1,0)
  
  x_test = as.matrix(test_dummies)
  y_test = ifelse(testing$pathology == "healthy", 1,0)
